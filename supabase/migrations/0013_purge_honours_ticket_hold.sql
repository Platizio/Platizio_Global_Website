-- 0013_purge_honours_ticket_hold.sql — close an asymmetry in the retention purge.
--
-- Found by running the purge against a fixture rather than by reading it.
--
-- 0011 gave complaints and tickets each their own legal_hold flag and let the
-- foreign key do the rest: a complaint that has not expired holds its ticket
-- back, because the reference is ON DELETE RESTRICT. That direction worked.
--
-- The other direction did not. A ticket placed under legal hold did nothing to
-- protect the complaint attached to it, so an expired grievance could be purged
-- out from under a ticket that was explicitly being preserved for the dispute
-- the grievance was about. The hold is placed on a matter, not on a row, and
-- the purge now reads it that way.
...
begin
  ...
  delete from public.complaints c
   where c.legal_hold = false
     ...
     -- A hold on the ticket is a hold on the grievance attached to it.
     and not exists (
