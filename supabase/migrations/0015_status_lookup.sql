-- 0015_status_lookup.sql — letting a customer check their own requests.
--
-- The reference issued at intake is only half a promise. `PG-2026-000001` is
-- something to quote in an email, but the customer cannot look it up, and the
-- indexes on ticket_ref and lower(requester_email) were put in at 0003
-- specifically for this.
--
-- WHY THIS IS NOT "ENTER YOUR TICKET NUMBER"
--
-- References come from a single sequence, so they are guessable by counting.
-- A page that takes a reference and shows the ticket would hand anyone the
-- name, email, mobile number and problem description of every customer who has
-- ever written in, by typing numbers in order. Adding "and your email address"
-- narrows it but still leaks: it turns into an oracle for whether a given
-- person has ever contacted Platizio.
--
-- So access is proven by control of the mailbox. You enter an email, we send a
-- short-lived link to that address, and the link is what opens the page. The
-- request endpoint answers identically whether or not that address has any
-- tickets, so it tells an attacker nothing either.
--
-- WHAT A CUSTOMER SEES
--
-- status_customer, never status_internal. That distinction is why 0001 created
-- two enums instead of one: "waiting on our broker partner" is real internal
-- state that must not leak counterparty workflow onto a public page. Nothing
-- here returns the assigned agent, the SLA deadlines, the submitting IP or any
-- internal note.
...
-- ---------------------------------------------------------------------------
--
-- The raw token is never stored. The Edge Function generates 32 random bytes,
-- sends the SHA-256 of them here, and puts the bytes themselves only in the
-- email. So a leak of this table yields no working links — the hashes cannot be
-- reversed into URLs.
--
-- One knowingly-accepted exposure: the outbox row for the email does contain
-- the live link in plain text, because the row *is* the email. That is bounded
-- by the same 30-minute expiry, and public.notifications is readable only by
-- SUPERVISOR and ADMIN — who can already read the tickets the link leads to.
-- Redacting it after send was considered and skipped: by the time any such job
-- could run the token has expired, so it would remove nothing that still works.
...
create table public.ticket_access_tokens (
  ...
  -- Deliberately reusable within its window rather than single-use. A one-shot
  -- link breaks on a refresh, on an email client that pre-fetches URLs, and on
  -- a customer opening it twice — and the failure looks like a bug, which
  -- pushes them to request more links. The expiry is what limits exposure.
...
-- Admins only. "How many links did this address request, and from where" is a
-- reasonable question during an abuse investigation; it is not something an
-- agent needs for day-to-day work.
create policy "admins read access tokens" on public.ticket_access_tokens
  for select to authenticated
  using ((select public.has_staff_role('ADMIN')));

-- ---------------------------------------------------------------------------
-- The outbox learns one more template
-- ---------------------------------------------------------------------------
...
-- Pure string building — no table reads, so no SECURITY DEFINER. It only ever
-- runs inside request_status_link, which is already definer.
...
-- ---------------------------------------------------------------------------
--
-- Returns the truth to its caller — the Edge Function needs to know whether it
-- queued anything so it can log it — and the Edge Function is what flattens the
-- response into something that says the same thing either way. Doing the lying
-- one layer up keeps this function honest and testable.
...
begin
  ...
  -- No tickets, no token, no email. The caller still reports success upwards;
  -- minting a token for an address with nothing behind it would only create a
  -- live credential that opens an empty page.
  if v_count = 0 then
  ...
  -- No dedupe_key: every request is meant to produce a fresh link, so these
  -- must not collapse into each other. The rate limit in the Edge Function is
  -- what stops this becoming a way to mail-bomb an address.
...
-- ---------------------------------------------------------------------------
-- lookup_tickets_by_token
-- ---------------------------------------------------------------------------
--
-- Every column returned here was chosen one at a time. The default is that a
...
declare
  n integer;
begin
  -- A day past expiry, not at expiry: the row is inert either way, and keeping
  -- it briefly means "was a link ever issued for this address" is answerable
  -- while an abuse report is still fresh.
